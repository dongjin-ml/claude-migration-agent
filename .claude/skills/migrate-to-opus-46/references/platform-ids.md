# Platform-specific model IDs and backend notes — Opus 4.6
...
|---|---|---|---|---|
| Anthropic API | `Anthropic()` | `claude-opus-4-6` (alias) or dated snapshot | `ANTHROPIC_API_KEY` | — |
| Google Vertex AI | `AnthropicVertex(project_id, region)` | `claude-opus-4-6@<YYYYMMDD>` — dated snapshot required; verify the exact ID enabled in the customer's Vertex Model Garden | ADC / service account (`GOOGLE_APPLICATION_CREDENTIALS`) | `ANTHROPIC_VERTEX_BASE_URL`, `ANTHROPIC_VERTEX_PROJECT_ID`, `CLOUD_ML_REGION`, `CLAUDE_CODE_USE_VERTEX` |
| AWS Bedrock | `AnthropicBedrock()` | `anthropic.claude-opus-4-6-<YYYYMMDD>-v1:0` (or the inference-profile ARN); verify the exact ID in the customer's Bedrock console | AWS credentials | `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `CLAUDE_CODE_USE_BEDROCK` |

> **Note:** The Anthropic API accepts the `claude-opus-4-6` alias. Vertex AI and Bedrock
...
| Backend | Opus 4.5 | Opus 4.1 |
|---|---|---|
| Anthropic API | `claude-opus-4-5-20251101` | `claude-opus-4-1-20250805` |
| Vertex AI | `claude-opus-4-5@20251101` | `claude-opus-4-1@20250805` |
| Bedrock | `anthropic.claude-opus-4-5-20251101-v1:0` | `anthropic.claude-opus-4-1-20250805-v1:0` |
